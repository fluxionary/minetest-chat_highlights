...
local t = {
	...
	},
	{
		"^(%S+) has fallen near (.*)$",
		function(name, text)
			return ("%s%s%s"):format(color_name(name), color_text(name, " has fallen near "), color_text(name, text))
		end,
	},
	{
		"^(%S+) has drown in ([^,]+), near ([^%.]+)%.",
	...
	},
	{
		"^(%S+) has drown in (.*)",
		function(name, what)
			return ("%s%s%s%s%s%s"):format(color_name(name), color_text(name, " has drown in "), color_text(name, what))
		end,
	},

	-- rollback_check messages
	{
		"%((%-?%d+,%-?%d+,%-?%d+)%) player:(%S+) (%S*) %-> (%S*) (%d+) seconds ago%.",
		function(pos, name, item1, item2, time)
			if item1 == "air" then
	...
	-- YL thankyous
	{
		"^Adventurer (%S+) received a 'Thank you' from (%S+)$",
		function(name1, name2)
			return ("%s%s%s%s"):format(
...
register_on_receive(safe(function(message)
	...
	for _, stuff in ipairs(t) do
		...
		if #parts > 0 then
			...
			if fmsg then
				fmsg = ("%s %s"):format(date, fmsg)

				if should_alert then
					minetest.sound_play("default_dug_metal")
					table.insert(alert_history, fmsg)
					should_alert = false
				end

				minetest.display_chat_message(fmsg)
				return true
			end
		end
	end
